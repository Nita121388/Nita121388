...
namespace Emoji.Wpf
{
    ...
    public class TextBlock : Controls.TextBlock, IEmojiControl
    {
        static TextBlock()
        {
            ...
            FontFamilyProperty.OverrideMetadata(
                ...
                new FrameworkPropertyMetadata(
                    (FontFamily)FontFamilyProperty.GetMetadata(typeof(Controls.TextBlock)).DefaultValue,
                    (o, e) => (o as TextBlock)?.OnFontFamilyChanged((FontFamily)e.NewValue)));
        ...
        private void OnFontFamilyChanged(FontFamily fontfamliy)
        {
            RecomputeInlines(Text, TextWrapping, fontfamliy.Source);
            EmojiInline.Refresh();
            OnColorBlendChanged(true);
        }
        ...
        private void RecomputeInlines(string text, TextWrapping wrapping,string fontfamliy = "")
        {
            Inlines.Clear();
            ...
            if (!string.IsNullOrEmpty(fontfamliy))
                EmojiData.ChangeFont(fontfamliy);
